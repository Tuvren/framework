...
**Authority:** PRD CAP-P0-070; ADR-0054 as amended by ADR-0056/ADR-0057, with ADR-0058 folded in pre-freeze.
**Audited commit:** `f64251ff` (master after Epic BJ #90 and Epic BK #91), on branch `feat/epic-bl-sdk-freeze-registry-publication`.
**Verdict:** the ADR-0057-amended stable-core surface is **freeze-ready**. All STOP conditions were checked and none tripped. The surface enumerated in §3 is the freeze candidate KRT-BL002 snapshots.
...
| --- | --- | --- |
| KRT-BJ003 host-import-contract boundary gate exists as an automated gate | PASS | `tools/scripts/host-boundary-gate.ts`; wired into `verify` (`tools/scripts/verify.ts:208`, id "ADR-0057 host import boundary gate") and into `check` (`tools/scripts/check.ts`, `INNER_LOOP_AUTHORITY_GATE_IDS`); root script `host-boundary:check` |
...
| `@tuvren/core/capabilities` | 22 | **wholly experimental** (ADR-0056 consistency floor applies) |
| `@tuvren/core/lifecycle` | 9 | stable |
| `@tuvren/core/security` | 12 | stable (net-new via KRT-BK004 — see §5.2) |
| `@tuvren/sdk` (root) | 116 | stable |

The Durable-Read Surface and `ExecutionHandle`/`awaitResult` surface are covered by `@tuvren/core/execution` (contract types) and `@tuvren/sdk` (re-exports); the published leaf packages are semver-guaranteed per ADR-0054 but not snapshot targets.

`@tuvren/sdk` additionally exposes an `./advanced` subpath (ADR-0059 escape hatch); it and the leaf packages are outside the KRT-BL002 snapshot per the ticket's scope.
...
- BJ004/BJ005: ADR-0058 destination contract, failure isolation (`safeDeliver`/`signalTelemetryFailure`), and the three funnel-isolation conformance checks all present.
...
5. **Tilde range enforcement.** All `@tuvren/core` peer ranges are `workspace:*` in-tree; the publish lane must materialize the ADR-0037 tilde range and fail if it cannot (the BJ008 release-lane validator currently also tolerates caret).
...
- `bun run check` — exit 0 at the audited commit (all authority gates incl. the ADR-0057 host boundary gate pass; affected typecheck/test/lint clean).

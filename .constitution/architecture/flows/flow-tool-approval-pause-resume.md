...
boundaries: [BND-01, BND-03, BND-04, BND-08, BND-06, BND-17, BND-18, BND-19]
view: sequence
certainty: assumed
...
Framework->>Runner: submit tool batch from current iteration
Runner->>Tooling: resolve, validate, and classify tools
...
Tooling-->>Runner: partial results + approval request for pending tools
Runner-->>Framework: resolution = pause(approval required)
...
Framework->>Runner: apply approval decisions and resume only unfinished approved or edited tool calls
Runner->>Tooling: continue execution
...
Framework-->>Host: continue same Turn without redoing completed side effects; rejection-only continuation policy remains host/runner owned
```

...
- **Risk (mitigated):** Content-funnel data leaks into the telemetry funnel (or a session comes to depend on the telemetry destination), collapsing the two-funnel separation.
- **Why it mattered:** In a split topology the telemetry destination is centralized while content is tenant-owned; content leaking into telemetry is a cross-boundary data exposure of the same severity class as a Scope leak, and a correctness dependency on the telemetry destination turns every telemetry outage into a session outage. The 2026-07-04 audit found provider wire data (request bodies, response headers) reaching durable state through a path that bypassed telemetry-grade screening (finding [B-01]).
- **Mitigation status:** Epic BJ's `KRT-BJ004`/`KRT-BJ005` shipped the Data Funnel Separation Model (ADR-0058: one-directional failure isolation, explicit-routing-only commingling) with three funnel-isolation conformance checks (destination-health equivalence, no-content-on-telemetry-by-default, delivery-failure-to-operational-signal). Epic BK's `KRT-BK004` then closed the specific leak the audit found: provider-bridge `bridgeExtras` values are now screened for secret-shaped content at the seam (`screenValueForSecretPatterns` in `@tuvren/core/security`) before reaching durable records or the canonical event stream, proven by a new pattern-shaped `secretPatternAbsence` conformance check. Both the structural funnel-separation model and the concrete audit-found leak are closed.

- **Risk:** Hosts (including the first-party Reference Host) import internal runtime machinery that a shared repository makes technically reachable, eroding the curated SDK boundary before it is ever frozen.
- **Why it matters:** Once downstream code depends on internal surfaces, the freeze either stabilizes accidents into the public contract or breaks consumers on day one; the audit found the Reference Host currently importing the internal runtime container directly in 14+ modules, so the erosion is present-tense, not hypothetical.
- **Mitigation or follow-up:** Enforce the host-facing tier rule (hosts consume the shared-primitive boundary, the convenience boundary, and chosen leaf containers only) with an automated boundary check in the verification path, and re-point the Reference Host at the curated surface before the freeze so it models what a real downstream host imports. The packaging realization is a TechSpec concern.

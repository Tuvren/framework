...
- **Risk:** Content-funnel data leaks into the telemetry funnel (or a session comes to depend on the telemetry destination), collapsing the two-funnel separation.
- **Why it matters:** In a split topology the telemetry destination is centralized while content is tenant-owned; content leaking into telemetry is a cross-boundary data exposure of the same severity class as a Scope leak, and a correctness dependency on the telemetry destination turns every telemetry outage into a session outage. Evidence of the leak direction already exists: the 2026-07-04 audit found provider wire data (request bodies, response headers) reaching durable state through a path that bypasses the telemetry-grade screening.
- **Mitigation or follow-up:** Apply the Data Funnel Separation Model (one-directional failure isolation, explicit-routing-only commingling) and extend conformance to assert both directions: no session behavior change when the telemetry destination is unavailable, and no content payload on the telemetry funnel absent an explicit routing decision. Screening applied where data crosses into either funnel belongs at one shared boundary module, not per-consumer.

...
| `@tuvren/session-client` real-socket, real-process M6 e2e (issue #102) | `@tuvren/session-client` (`typescript/host/session-client`) and the REPL host's `--serve-ws` mode are TypeScript-only host/reference-peer implementations riding on the portable `tuvren.framework.host-session` and `tuvren.framework.event-stream-ws` wire vocabularies, not new protocol authority. A real OS process and a real WebSocket are not available to the in-process conformance harness, so the strongest claims this e2e proves (a genuinely dropped socket redelivering across `@tuvren/remote-session`'s reattach, and a durably committed capability result surviving a real `SIGKILL` of the server process) stay package-test-only evidence; the conformance-plan equivalents (`host-session.reattach-redelivers-unanswered-invocation` et al., `az.az006.network-reconnect-redelivery-dedup`, `az.az006.network-kill-durability-recovered`) prove the same mechanics through the real binding -> session -> transport composition in-process, honestly named for that narrower scope. | `typescript/host/repl/test/repl-serve-ws.e2e.test.ts` (Variant A: socket-kill mid-dispatch redelivery; Variant B: durable commit survives a real SIGKILL, run against a disposable PostgreSQL schema); `typescript/host/remote-session/test/remote-client-session.test.ts`; `typescript/host/session-client` workspace tests. |

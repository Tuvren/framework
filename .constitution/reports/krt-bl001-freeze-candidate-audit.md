...
| No Durable-Read or `ExecutionHandle`/`awaitResult` signature requires a caller-supplied scope parameter (ADR-048) | PASS | `TuvrenRuntime` read methods (`getThread`, `getTurnHistory`, `getTurnState`, `listBranches`, `listThreads`, `readBranchMessages`) and `ExecutionHandle.awaitResult()` / `OrchestrationHandle.awaitResult()` in `typescript/core/src/lib/runtime-contract-shapes.ts:1196–1420` — zero `scope` parameters anywhere on the public read surface |

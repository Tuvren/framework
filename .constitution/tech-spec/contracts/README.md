...
- **Ownership:** the host owns the *policy*; the framework owns only the *seam and its ordering*. `sanitizeToolResult?: (result: ToolResultPart, ctx: { callId, executionClass, toolName }) => ToolResultPart` is supplied on `AgentConfig` / `RuntimeCoreOptions`, threaded onto `ToolBatchEnvironment`, and applied inside `stageAndEmitResult` (`typescript/runtime/src/lib/tool-execution-helpers.ts`) — the sole chokepoint preceding both durable staging and the canonical `tool.result` event. Complements, and does not replace, the §5.6.3 framework-enforced secret isolation (ADR-044), which continues to own credentials the framework itself introduced.

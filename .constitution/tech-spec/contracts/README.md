...
> **Authority note:** This file is the companion index for the sidecars in this directory, migrated from `TechSpec.md §4`. It states purpose and invariants. It is not a contract artifact. Each sidecar's `artifact_path` points at the boundary-owned source (TypeSpec `main.tsp`, generated JSON Schema, `.proto`, or an authority-packet manifest under `spec/<port>/`). Per ADR-0023/ADR-0024/ADR-0025 and the authority-packet `forbiddenAuthoritySources`, the constitution is never the cross-implementation contract oracle.

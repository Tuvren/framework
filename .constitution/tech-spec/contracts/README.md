...
- The `ClientEndpointBoundary` lease counter remains process-local by decision (ADR-063 §6): cross-invocation uniqueness comes from `callId`, and the one cross-restart token collision this admits implies the same logical invocation, whose side-effect-once guarantee is the ADR-052 idempotency envelope's job rather than the lease token's.
...
- **Ownership:** the host owns the *policy*; the framework owns only the *seam and its ordering*. `sanitizeToolResult?: (result: ToolResultPart, ctx: { callId, executionClass, toolName }) => ToolResultPart` is supplied on `AgentConfig` / `RuntimeCoreOptions`, threaded onto `ToolBatchEnvironment`, and applied inside `stageAndEmitResult` (`typescript/runtime/src/lib/tool-execution-helpers.ts`) — the sole chokepoint preceding both durable staging and the canonical `tool.result` event. Complements, and does not replace, ADR-044 §5.6.3 framework-enforced secret isolation, which continues to own credentials the framework itself introduced.

...
> **Authority note:** This file is descriptive documentation migrated verbatim from `TechSpec.md §4`. The authoritative raw interface contracts are boundary-owned (TypeSpec `main.tsp`, generated JSON Schema, `.proto`, and language bindings under `spec/<port>/`, e.g. `spec/core/`, `spec/kernel/`, `spec/providers/`, `spec/tools/`, `spec/runners/`, `spec/host/`, `spec/interop/`). Per ADR-0023/ADR-0024/ADR-0025 and the authority-packet `forbiddenAuthoritySources`, the constitution is never the cross-implementation contract oracle — it points to boundary authority rather than duplicating raw contract files.
...
- **Ownership:** `@tuvren/core/errors` (per ADR-0037) owns the shared error base class and category subclasses. Concrete packages own their package-specific `code` values and message text. The deprecated `@tuvren/core-types` re-export shim completed its one-cycle window and was removed (KRT-BM006).
...
- **Package partition note:** Per ADR-0037, the merged `spec/core/authority-packet.json` is the single machine authority anchor for the shared framework runtime semantics, host-facing runtime surface, event vocabulary, tool contracts, runner contracts, and provider contracts. `@tuvren/core` exposes these through subpath exports (`/messages`, `/tools`, `/events`, `/errors`, `/execution`, `/runner`, `/provider`, `/extensions`, `/telemetry`, `/capabilities`, `/lifecycle`); the historical `@tuvren/runtime-api`, `@tuvren/event-stream`, `@tuvren/tool-contracts`, and `@tuvren/driver-api` packages have been fully absorbed and removed (the Epic 87 M6 driver→runner rename retired `@tuvren/driver-api` into `@tuvren/core/runner`). `@tuvren/core-types` completed its deprecation window and was removed (KRT-BM006). `@tuvren/runtime` is the slim convenience package exposing `createTuvren({...})` plus curated re-exports.
- **Durable-read note:** Per ADR-0036, the `TuvrenRuntime` interface now exposes `listThreads`, `listBranches`, `getTurnState`, `getTurnHistory`, and `readBranchMessages` as host-facing durable-read operations that compose existing kernel structural primitives plus the new `thread.list` syscall (ADR-0034). Pagination follows the Architecture §6 rule: history surfaces use cursor + async iterator (`getTurnHistory`); collection surfaces use cursor + optional limit (`listThreads`, `readBranchMessages`). Exception: `listBranches` is intentionally unbounded because branches per thread are bounded by O(1) active divergence paths in v1 and the underlying `kernel.branch.list` primitive is itself unpaginated. Cursors are opaque to the host; their runtime structure is specified in §3.8.
- **Handle terminal-value note:** Per ADR-0035, `ExecutionHandle` now exposes `awaitResult(): Promise<ExecutionResult>` on the base interface. `OrchestrationHandle.awaitResult()` overrides to return `OrchestrationResult` (a type intersection `ExecutionResult & { childResults: Record<string, ExecutionResult> }`) with subtree-aggregated final values.
...
export interface ExecutionHandle {
  ...
  // ── Terminal-value surface (ADR-0035) ─────────────────────────────
...
export interface RuntimeKernel {
  ...
  thread: {
    ...
    // ADR-0034: capability-advertised; rejects with
    // TuvrenPersistenceError code "kernel_capability_unsupported"
...
service KernelThreadService {
  ...
  // ADR-0034: capability-gated thread enumeration. The remote kernel
...
- A raw `pass` status is necessary but not sufficient for TypeScript readiness or future implementation-line activation. Sufficiency is defined by the active staged gates from ADR-0033 and `§5.4`: product-proof evidence, platform evidence, portability evidence, and any current clean-checkout verification checks named by the live build sequence.
...
- **Error model:** Manifest-validation failures, missing declared sources, unreachable generated artifacts, undeclared generated outputs, and references to declared forbidden authority sources stop CI through the §ADR-0027 freshness gate.
...
- Event observations are ordered neutral event arrays; per ADR-0025 the runner asserts the event sequence rather than the adapter implementing the expected sequence.
...
// Detection precedence (ADR-0038): wrapped (schemaSymbol) → Zod v4
...
- The runtime emits to the configured sink at: turn/run start and end, iteration boundaries, model request/response, tool call start/end and approval transitions, checkpoint commit, recovery resume-or-fail, bounded-execution stop (ADR-0043), and errors. Emission reuses the canonical event vocabulary so the telemetry surface and the real-time host stream cannot diverge.
...
- No telemetry record may carry secret material (ADR-0044); host-supplied attributes pass through a semconv allowlist, secret-like values on otherwise allowed keys are dropped or sanitized, and any `TelemetrySpan.error.message` is a sanitized runtime summary rather than a raw upstream error string.
...
- **Compatibility Strategy:** §2.1 execution bounds compatibility. Reuses the `failed` `ExecutionResult` discriminant from ADR-0035; no new union variant.
...
- **Style:** versioned wire message family, TypeSpec-to-JSON-Schema authority (ADR-0060)
...
- **Style:** wire-level sequencing envelope + opaque cursor token, TypeSpec-to-JSON-Schema authority (ADR-0061)
...
- **Compatibility Strategy:** additive experimental surface per ADR-0056 (`0.x` packet, `@experimental` exports). Cursor payload carries `v: 1`; an unknown version decodes to out-of-window (snapshot fallback), never a wire error.
...
- Transport projections: SSE places the cursor in the frame `id` (WHATWG `Last-Event-ID` reconnection); the WebSocket transport (ADR-0062, #100) carries it on its outer envelope and handshake.

### 4.24 WebSocket Transport Carriage Contract (`spec/streaming/ws/`)

- **Style:** JSON text envelopes over an abstract socket seam, TypeSpec-to-JSON-Schema authority (ADR-0062)
...
- **Style:** TypeScript binding-only lifecycle seam over the §4.22 frame vocabulary; no wire schema of its own (ADR-0063)
...
- **Compatibility Strategy:** additive experimental surface per ADR-0056 (`@experimental` exports, `private` package). Introducing it is a breaking change to `@tuvren/stream-ws`, which drops its internal per-connection sequencer and takes a session rather than a binding — anticipated by ADR-0062 and permitted by that package's `0.x` posture.
...
- Redelivery is at-least-once *presentation* and relies on `ClientInvocationEnvelope.idempotencyKey` (ADR-0052) for at-most-once *effect*. At most one sink is attached at a time; a second concurrent `attach` is a programming error, not a second consumer.
- The `ClientEndpointBoundary` lease counter remains process-local by decision (ADR-0063 §6). Its staleness job is to reject a result produced for a superseded dispatch *within one process*, which a process-local counter does completely. The cross-restart token collision it admits is real: a resumed run re-presents the same `callId`, so a fresh boundary can mint a token matching a pre-restart one and accept that late result. It is accepted because the two dispatches are the same logical invocation and ADR-0065 guarantees both carried an identical idempotency key, so a peer that presents the key performs the effect once and the committed result describes that one effect. The window exists only for retryable work still in flight at process death — `nonRetryable` invocations are not re-run under a dead owner (ADR-0052 §2) and completed results are recovered by `callId` rather than re-dispatched.

### 4.26 Tool-Result Sanitization Seam (host-supplied)

- **Style:** optional host-supplied synchronous hook on the runtime configuration surface (ADR-0064)
- **Ownership:** the host owns the *policy*; the framework owns only the *seam and its ordering*. `sanitizeToolResult?: (result: ToolResultPart, ctx: { callId, executionClass, toolName }) => ToolResultPart` is supplied on `AgentConfig` / `RuntimeCoreOptions`, threaded onto `ToolBatchEnvironment`, and applied — via the shared pure helper `applySanitizeHookToPart` — at every path that can durably stage a tool result before it does so: `stageAndEmitResult` (`typescript/runtime/src/lib/tool-execution-helpers.ts`), the Tool Execution Gateway chokepoint preceding both durable staging and the canonical `tool.result` event for gateway-dispatched results, and the pre-staged provider tool-message path (AY003) in `executeIterationPhase` (`typescript/runtime/src/lib/runtime-core-iteration.ts`), which never passes through the gateway. Complements, and does not replace, the §5.6.3 framework-enforced secret isolation (ADR-0044), which continues to own credentials the framework itself introduced.
...
- Verified by asserting the scrubbed form against **persisted kernel records and captured canonical events**, never against the hook's return value — following ADR-0044 §4's rule that absence of secret material is asserted, not assumed.

...
- **Batteries-included composition compatibility:** Per ADR-040, adding a new `BackendKind` or `RunnerKind` is semver-minor; changing the default `runner` is semver-major; renaming `CreateTuvrenOptions` fields is semver-major.
...
- **Operational telemetry compatibility:** Per ADR-042, adding the `@tuvren/core/telemetry` subpath and the `TuvrenTelemetrySink` interface is semver-minor. The canonical telemetry vocabulary (`spec/telemetry/semconv/tuvren-runtime.yaml`) versions independently as boundary-owned authority; adding semconv attributes is minor, removing or renaming one is major. `@tuvren/telemetry-otel` is an implementation-specific projection that follows its own semver and is not part of the portable cross-language surface.

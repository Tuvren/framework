...
- **Touched Files (out of the declared In-Scope list):**
  ...
  - `typescript/sdk/src/lib/payload-codec.ts` — dropped the relocated identity-codec definitions; imports the discriminant/guard from `@tuvren/core/lifecycle` and re-exports the identity helpers for surface continuity; keeps only the batteries-included `createAesGcmPayloadCodec`.

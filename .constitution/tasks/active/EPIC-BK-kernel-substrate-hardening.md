...
- **Scope-text corrections (verified against running code and the actual plan JSON, not assumed):**
  ...
  4. **`kernel.scope-isolation.*` remains correctly deferred**, so Gherkin Scenario 1's literal text ("scope-isolation checks run against rust-kernel") does not hold as written — only reclamation and run-liveness were promoted this ticket. Reading `rust/kernel/src/memory.rs`'s `KernelState` confirmed zero Scope dimension exists anywhere in the Rust kernel (unlike the restart-af/erasure cases above, this is a genuine, honest capability absence, not an identity-based exclusion — the Rust kernel cannot express scope-confinement today). A follow-up ticket for `kernel.scope-isolation` promotion (threading a new Scope dimension through ~30 `InMemoryKernel` methods) is filed as part of this epic's closure, mirroring how restart/crash-recovery checks already defer to Epic BN.

...
##### KRT-BL004 Review Note
Independent single-pass review (2026-07-11) found one P2: the guide overclaimed the mechanical boundary gate's coverage (it forbids `@tuvren/runtime` + the two kernel packages, not `provider-api`/`telemetry-semconv`, and the Reference Host legitimately imports `telemetry-semconv`). Fixed in-milestone by scoping the enforcement claim to the three gate-forbidden packages and keeping the other two as avoid-by-contract guidance; `@tuvren/backend-shared` reworded as the backends' transitive support package. All touched files were in the ticket's declared scope; no deviation entry required.

## Close-Out (2026-07-11)

All four tickets completed, each milestone committed and independently reviewed with blocking findings fixed before the next milestone began. Delivered state: the ADR-057-amended stable-core surface is audited (`.constitution/reports/krt-bl001-freeze-candidate-audit.md`), snapshotted, and gate-enforced (`tools/scripts/api-freeze-gate.ts` + committed baseline, wired into `check`/`verify`); the 19-package curated set is live on registry.npmjs.org at `0.1.0` with npm provenance, ADR-037 tilde `@tuvren/core` peers, and the five-package engine tier (`runtime`, `kernel-protocol`, `kernel-runtime`, `provider-api`, `telemetry-semconv`) published-internal per ADR-057 item 5; a fresh-install consumer completed a first Turn via `createTuvren` from the published `@tuvren/sdk`; and adopter onboarding ships at `docs/guides/publishing-and-adopter-onboarding.md` with its walkthrough executed verbatim against the live registry.

### Outgoing Hand-Offs

1. **npm trusted-publishing migration (mandatory, deadline January 2027).** The first publish used the one-shot `NPM_TOKEN` bypass-2FA granular token via `.github/workflows/release.yml`. npm's 2026-07-08 policy change ends direct publishing with such tokens in January 2027. Configure GitHub Actions trusted publishing (OIDC) on npmjs.com for **all 19 published packages**, then delete the `NPM_TOKEN` repository secret; the workflow already carries `id-token: write` and needs no changes. Operator notes in `release.yml` record the same obligation.
2. **TypeScript 7 dual-pin toolchain migration (proposed for Epic BM or a successor).** `typescript@7.0.2` (the native Go compiler, ~10x faster) is stable but binary-only with no JS compiler API; the freeze gate, host-boundary gate, and `tsup` all consume the 6.x compiler API. Proposal: dual-pin — tsc 7 for `typecheck` lanes, 6.x retained for compiler-API tooling — assessed and deferred during this epic.
3. **Reference-host `runtimeVersion` label drift.** `typescript/host/repl` hardcodes `"@tuvren/runtime@0.0.0"` as its transcript label (`src/lib/cli.ts:477`, `src/lib/repl-replay.ts:455`). Writer and comparator share the constant so nothing fails, but the label is stale post-`0.1.0`.
4. **Doc drift:** `spec/host/client-endpoint-integration.md:129` shows an example importing `@tuvren/runtime`, contradicting the ADR-057 host import contract that page should model.
5. **BL002 residual:** the freeze gate expands structural signatures only for types declared in-repo; a stable export whose shape changes solely through an out-of-repo reference type would not diff. Accepted as a known blind spot in the KRT-BL002 review; noted for a future gate hardening pass.
6. **Reference-host `telemetry-semconv` dependency.** The repl imports `@tuvren/telemetry-semconv` (`repl-types.ts:31`, `repl-scenarios-support.ts:31`), which BL001 §2.6 retracted to published-internal. Legal today (the boundary gate only forbids runtime/kernel imports) but worth re-pointing or gate-extending so the first-party host models the full avoid-by-contract guidance.

...
##### KRT-BK002 Deviations & Justifications
- **Touched Files:** `typescript/kernel/backends/memory/src/lib/memory-backend.ts`, `typescript/kernel/backends/sqlite/src/lib/sqlite-backend.ts`, `typescript/kernel/backends/postgres/src/lib/postgres-backend.ts`, `typescript/kernel/backends/postgres/src/lib/memory-backend-reclamation.ts`, `typescript/kernel/backends/shared/src/index.ts`, `typescript/kernel/backends/shared/src/lib/backend-invariant-record-utils.ts`, `typescript/kernel/backends/shared/test/backend-invariant-reclamation.leaseless-expiry.test.ts`, `typescript/kernel/conformance-adapter/src/host.ts`, `typescript/kernel/protocol/src/lib/kernel-types.ts`
- **Justification:** The leaseless-expiry rule needs a wall-clock reference (`nowMs`) inside `reclaimBackendState`, which the protocol layer already anticipated: `ReclamationOptions.nowMs` and the `maintenance.reclaim()` kernel-level wrapper already threaded a clock through to `backend.reclaim()` before this ticket, but all three backends' `reclaim()` methods silently discarded it. Wiring it up required each backend's `reclaim()` call site (`memory-backend.ts`, `sqlite-backend.ts`, `postgres-backend.ts`) to pass `options?.nowMs ?? this.now()` through to the shared core, plus postgres's own copy of the reclamation shim (`memory-backend-reclamation.ts`) to accept and forward it. Postgres is outside this ticket's required Gherkin proof (only memory and sqlite are asserted against), but leaving its already-injectable `this.now()` clock unconsumed while memory/sqlite consume theirs would silently break parity across the shared-core backends established in KRT-BK001 — postgres's own leaseless runs would simply never expire. The new `isExpiredLeaselessRunningRun` predicate and its `LEASELESS_RUN_EXPIRY_MS` default land in `backend-invariant-record-utils.ts` (the post-KRT-BK001 shared-core successor to the ticket-listed `memory-backend-record-utils.ts`, per the parenthetical already granted for the sibling reclamation file), with its own dedicated unit-test file; `backend-shared/src/index.ts` gained two new exports (`isExpiredLeaselessRunningRun`, `LEASELESS_RUN_EXPIRY_MS`) so both are consumable outside the package. `kernel-types.ts`'s `ReclamationOptions.nowMs` JSDoc explicitly claimed "the reference memory backend... does not consult nowMs" — now false, so the comment was corrected (no interface shape change). `host.ts` gained two new probe blocks so the ticket's own Gherkin Scenario 3 ("the leaseless-expiry check passes... bun run verify:kernel") has a live conformance operation to assert against, per the plan's two new `kernel-reclamation.json` checks. Separately, `typescript/kernel/runtime/src/lib/runtime-kernel-runs.ts` (`create`, line ~273) was declared in-scope anticipating a possible lease-requirement policy change to the run-creation call shape; the team resolved the ticket's own STOP condition by choosing the administrative-timeout expiry path over requiring leases on every abandonable run, which needs no change to `create()`'s existing call shape — so this file carries zero diff by design, not by omission.
...
##### KRT-BK004 Deviations & Justifications
- **Touched Files:** `typescript/core/src/lib/secret-pattern-screening.ts` (new), `typescript/core/src/security/index.ts` (new), `typescript/core/package.json`, `typescript/core/tsup.config.ts`, `typescript/core/test/secret-pattern-screening.test.ts` (new), `tsconfig.base.json`, `typescript/runtime/tsconfig.typecheck.json`, `typescript/runtime/tsconfig.dts.json`, `typescript/providers/bridge-ai-sdk/tsconfig.typecheck.json`, `typescript/providers/bridge-ai-sdk/tsconfig.dts.json`, `typescript/providers/certification/tsconfig.typecheck.json`, `typescript/runners/react/tsconfig.typecheck.json`, `typescript/providers/bridge-ai-sdk/test/ai-sdk-provider-bridge-secret-screening.test.ts` (new), `tools/conformance/plan-compiler/index.ts`, `tools/schemas/conformance-plan.schema.json`, `tools/conformance/harness/secret-absence/index.ts`, `tools/conformance/harness/test/secret-absence.test.ts`, `tools/conformance/harness/assertion-engine/index.ts`, `tools/conformance/harness/test/assertion-engine.test.ts`, `typescript/conformance-adapter/src/framework-adapter.ts`, `spec/conformance/telemetry/plans/framework-operational-telemetry.json`, `spec/core/authority-packet.json`, `spec/providers/authority-packet.json`.
- **Justification:** Three groups of out-of-scope touches, each necessary to satisfy the ticket's own Description/Gherkin without weakening screening semantics.

  First, the shared regex-based secret-detection primitives (`SECRET_KEY_PATTERN`, `URL_CREDENTIAL_PATTERN`, `CONNECTION_STRING_PATTERN`, `AUTH_HEADER_PATTERN`, `CREDENTIAL_ASSIGNMENT_PATTERN`, `JWT_PATTERN`, `LONG_SECRETISH_PATTERN`, `isSecretLikeKey`, `sanitizeSecretLikeText`) had to move out of `typescript/runtime/src/lib/telemetry-secret-screening.ts` into a new `@tuvren/core/security` subpath rather than being duplicated or imported cross-sideways, because the AI SDK provider bridge (`typescript/providers/bridge-ai-sdk`) does not and must not depend on `@tuvren/runtime` (that would invert the framework's existing dependency direction — runtime depends on the provider bridge, not the reverse), while `@tuvren/core` is the one package both `@tuvren/runtime` and `bridge-ai-sdk` already depend on non-optionally today. Landing the shared primitives in a new focused `@tuvren/core/security` subpath (mirroring the existing `@tuvren/core/lifecycle`/`@tuvren/core/errors` subpath convention) let both call sites consume the identical pattern set with no new cross-package dependency edge. This required `typescript/core/package.json`'s `exports` map and `typescript/core/tsup.config.ts`'s entry array to register the new subpath, `tsconfig.base.json` plus every downstream project's own `tsconfig.typecheck.json`/`tsconfig.dts.json` (runtime, bridge-ai-sdk, providers/certification, runners/react — the exact set of projects whose typecheck graph transitively includes `@tuvren/runtime` or `@tuvren/provider-bridge-ai-sdk` source) to resolve the new path alias, and a new unit-test file (`typescript/core/test/secret-pattern-screening.test.ts`) proving the relocated primitives behave identically in their new home. `typescript/runtime/src/lib/telemetry-secret-screening.ts` now imports `isSecretLikeKey`/`sanitizeSecretLikeText`/`REDACTED` from `@tuvren/core/security` instead of defining them locally; its own public contract (exported function signatures and behavior — `filterTelemetryAttributes`, `isTelemetryAttributeAllowed`, `sanitizeTelemetryErrorSummary`, `sanitizeTelemetryAttributeValue`) is unchanged, verified by running its existing test suite (`test/runtime-core.telemetry.test.ts`) against both the pre-change and post-change source: both report an identical 15 pass / 0 fail / 54 expect() calls, honoring the ticket's STOP condition against relaxing or altering the screening module's own contract.

  Second, the new `"secretPatternAbsence"` conformance assertion kind required additive-only changes across `tools/conformance/plan-compiler/index.ts` (the `AssertionKind` union), `tools/schemas/conformance-plan.schema.json` (enum entries plus a new `allOf` conditional block requiring `field` instead of `secretsPath`, mirrored at both the top-level and step-level assertion schema locations), a new sibling `findSecretPatternLeaks`/`hasSecretPatternResidue` pair alongside the existing `findSecretLeaks` in `tools/conformance/harness/secret-absence/index.ts`, and a new `case "secretPatternAbsence"` in `tools/conformance/harness/assertion-engine/index.ts`'s `evaluateAssertion` switch. None of these touches are declared in-scope by the ticket, but the ticket's own third Gherkin scenario ("Pattern-shaped secret-isolation conformance passes... the secret-isolation check detects the credential-shaped leak class") is unsatisfiable without a structural (pattern-based) assertion kind distinct from the pre-existing value-equality `secretAbsence` kind, since the new fixture value (`bridgeRequestToken`, a JWT-shaped but not-exactly-configured secret) is deliberately not present verbatim anywhere the existing `secretAbsence` assertion checks. Every one of these changes is strictly additive: existing `secretAbsence`-kind code paths, schema branches, and `findSecretLeaks`/`buildHaystack`/`collectStrings` functions are untouched, and this was verified both by full test-suite passes (`tools/conformance/harness/test/secret-absence.test.ts`, `tools/conformance/harness/test/assertion-engine.test.ts`, each extended with new `describe` blocks rather than modified existing ones) and by the overall conformance run showing zero regressions (see below).

  Third, exercising the AI SDK provider bridge directly required a new conformance-adapter operation (`runtime.secret-isolation.provider-bridge`, wired into `typescript/conformance-adapter/src/framework-adapter.ts`'s operation switch) plus its implementation in the in-scope `framework-adapter-secret-isolation.ts`, and a companion plan check in `spec/conformance/telemetry/plans/framework-operational-telemetry.json` (planVersion bumped `0.4.0` → `0.5.0`, mirrored in `spec/core/authority-packet.json`'s `conformancePlans` catalog entry for the same plan) with two new assertions (`secretAbsence` and `secretPatternAbsence`) against both the bridge's `generate()` and `stream()` outputs. This operation deliberately calls `createAiSdkProviderBridge({ model: <locally-constructed fake LanguageModelV3-shaped mock> }).generate()`/`.stream()` directly — it does not route through a kernel run, a runner, or a durable run-record round-trip — because the ticket's scope is the bridge seam itself (`buildProviderMetadata`'s `aiSdkBridge` field), and a full persisted-run-record path would conflate this ticket's seam-level proof with the pre-existing kernel/runtime persistence conformance surface that already covers durable-record shape elsewhere. `spec/providers/authority-packet.json` gained a new `bindingSections.secret-isolation` narrative entry (this file had no `bindingSections` key before) describing both the pre-existing value-equality secret-isolation precedent and this ticket's new pattern-based screening/detection work, mirroring the equivalent KRT-BK002 precedent of keeping capability narratives current in `spec/kernel/authority-packet.json`; validated schema-clean via `tools/scripts/authority-packet/validate-authority-packets.ts`. The full `bun run conformance` gate went from the pre-change baseline (466 passed / 0 failed / 12 non-applicable / 478 total, exit 0) to 467 passed / 0 failed / 12 non-applicable / 479 total, exit 0 after this ticket's changes — exactly one new check added, zero regressions, and the non-applicable count unchanged (the new check is non-applicable to the batteries-included adapter for the same reason its sibling secret-isolation checks already are).

  **Post-review fixes (dual-axis standards + spec review, both non-blocking P2, fixed before the next milestone):** (1) `tools/conformance/harness/secret-absence/index.ts` line 44's `PART_SEPARATOR` constant held a literal NUL byte instead of a real space — a pre-existing defect from an earlier commit that made this exact file render as a git "binary" diff, so this milestone's own ~90-line addition to it (`findSecretPatternLeaks`/`hasSecretPatternResidue`) was unreviewable via normal `git diff`/PR tooling; fixed to a real space now that this milestone was already touching the file. (2) The structural pattern list was duplicated between `sanitizeSecretLikeText` (`@tuvren/core/security`, a hard-coded OR-chain) and `secret-absence/index.ts`'s `SECRET_STRUCTURAL_PATTERNS` (an independent re-listing of the same six regexes) — a latent coverage-drift risk where a future 7th pattern added to the screen's OR-chain could silently fail to reach the conformance detector. Both now derive from one new exported `SECRET_VALUE_PATTERNS` ordered list in `@tuvren/core/security`, making the "detector can never fall behind the screen" claim structurally enforced rather than convention-dependent. (3) `bridgeRequestToken` had been added to both its own fixture field and the `secretValues` array, which both reviewers independently flagged as self-contradicting the ticket's own Gherkin intent ("a credential-shaped value not equal to any configured secret") and as making the new `secretPatternAbsence` assertion redundant with the pre-existing `secretAbsence` assertion for this specific check, since a value present in `secretValues` is by definition a *configured* secret. Removed it from `secretValues` (kept only as its own named fixture field) so `secretPatternAbsence` is the sole, genuinely load-bearing detector for it, and corrected the fixture's `$comment` to state this explicitly. Re-verified after all three fixes: `bun run conformance` still 467 passed / 0 failed / 12 non-applicable / 479 total, exit 0; `bun run typecheck` and `bun run lint` both exit 0; all affected unit/conformance-harness test suites green.
...
Feature: Kernel input hardening
  ...
  Scenario: gRPC service enforces a message-decode-size ceiling
    ...
    Then the server rejects it with an out-of-range error (tonic's real status for an over-ceiling decode; verified against tonic 0.14.5's `codec/decode.rs`)
...
- **Timeout/concurrency proof (closed a P1 found in milestone review):** the spec-axis review round rejected the initial "known test gap" framing above as an unauthorized skip of a required Gherkin scenario (no "use your judgment" allowance exists in this ticket's text, and this same commit already set a higher bar for the decode-size scenario). Closed by extracting `serve_kernel_grpc`'s cap-configuration into a shared `configured_server_builder(timeout, max_concurrent_requests_per_connection)` helper (production always calls it with the real 30s/64 constants) and adding a test-only `artificial_delay` field to `KernelGrpcServiceImpl` (defaulting to `None`, a no-op in production, mirroring the existing `max_walk_back_hops` test-injection precedent). Two new `#[cfg(test)]` tests in `rust/kernel-grpc-service/src/lib.rs` drive a real server and real tonic client at small test values and prove both ceilings actually fire: `.timeout(..)` removal was verified to turn the timeout test red (mutation-tested directly), and `.concurrency_limit_per_connection(..)` removal was verified to turn the concurrency test red. Confirmed via tonic 0.14.5 source that a timeout expiry surfaces as `Code::Cancelled` (not `Code::ResourceExhausted`) and that the concurrency limit serializes excess requests rather than rejecting them (tonic never enables `.load_shed(true)` here).
...
**Post-review fix (dual-axis standards + spec review, non-blocking Investigate finding, fixed before the next milestone):** The standards-review agent live-re-ran the committed `sqlite-load-cost.bench.ts` script and reproduced an increasing-multiplier growth trend but with different intermediate values (~4.0x, ~10.9x through 10→100→1,000) than the committed report's (3.43x, 9.58x), and flagged that the report's "accelerating / worse-than-O(n)" framing for SQLite's `health()`/`reclaim()` growth was stated with more confidence than 4 data points at 5 samples each (where p95 is mathematically indistinguishable from max) actually support, given the decisive 1,000→10,000 jump is a single interval and a memory/GC/allocation effect around the ~20k in-memory-record mark was never ruled out. The spec review separately confirmed Gherkin Scenarios 1 and 2 both pass and raised only two minor general-feedback notes (no per-sample `releasedObjectCount` retained; the sqlite table's "DB size (turn nodes)" axis label under-counts total loaded rows by the equal-sized orphan batch), with no P0/P1/P2 from either axis. Per this epic's practice of proactively fixing cheap, clearly-correct non-blocking findings: reworded `SPK-BK007.md` §2/§3/§4 (the four passages the standards reviewer cited) to present the SQLite top-of-range growth jump as a hypothesis warranting its own follow-up investigation — explicitly naming the sample-size, p95-at-n=5, and memory/GC-confound caveats — rather than as a settled algorithmic-complexity conclusion, and clarified that the primary Option-B recommendation does not depend on that secondary claim (it holds under the best-case/minimum sample at every tier for both backends). Also added the spec review's suggested axis-label clarification to the SQLite results table. No script or Nx-target change was needed — both bench scripts' own logic was independently confirmed correct by both reviewers (live re-execution, cross-checked against the actual `postgres-backend-persistence.ts`/`sqlite-backend.ts` code paths); only the spike report's prose was overclaiming relative to its own evidence. Re-verified: no residual placeholder text, both Gherkin scenarios still satisfied, zero `src/**` diffs unchanged.
...
##### KRT-BK008 Deviations & Justifications
- **Touched Files (anticipated call-site threading):**
  - `typescript/kernel/runtime/src/lib/runtime-kernel-lineage.ts` — `createTurnTree` gained an optional `priorTurnTreeHash` parameter; `createIncorporatedTree` threads its own `baseTurnTreeHash` through it.
  - `typescript/kernel/runtime/src/lib/runtime-kernel.ts` — `tree.incorporate` threads `baseTurnTreeHash` through as `priorTurnTreeHash`; `tree.create` deliberately does **not** (see below).
  - **Justification:** `toStoredTurnTreePath`'s only caller is `createTurnTree`, which itself has four call sites (`thread.create`, `tree.create`, `tree.incorporate`, `createIncorporatedTree`). The chunk-aware optimization is only safe for callers that build `changes` via `applyStagedResultsToManifest` (structurally append-only) — that's `tree.incorporate` and `createIncorporatedTree`. `tree.create` merges caller-supplied `changes` (`{ ...baseManifest, ...changes }`) with no append-only guarantee, so it is deliberately excluded and left on the unoptimized flat path, with an explanatory comment at that call site so it isn't "fixed" into a bug later. `thread.create` builds a fresh empty manifest and has no prior tree, so it was never a candidate.
- **Touched Files (new tests, exceeding the ticket's declared Scope):**
  - `typescript/kernel/runtime/test/runtime-kernel-storage.chunk-aware.test.ts`, `runtime-kernel-storage.chunk-constant-agreement.test.ts`, `runtime-kernel.turn-tree-chunking.test.ts`, `runtime-kernel-lineage.apply-staged-results.test.ts`, and an additive `backend?: RuntimeBackend` override parameter on `runtime-kernel-test-helpers.ts`'s `createThreadFixture`.
  - `typescript/kernel/backends/sqlite/test/backend-sqlite.turn-tree-chunk-aware-writes.test.ts` and `typescript/kernel/backends/postgres/test/backend-postgres.turn-tree-chunk-aware-writes.test.ts` — cross-backend parity proof for the same optimization.
  - **Justification:** TDD test strategy per the epic's testing rules. The sqlite/postgres parity tests live in those packages (not `kernel-runtime/test`) because both already devDepend on `@tuvren/kernel-runtime` for their own kernel-level tests — adding the reverse edge would create a real Nx project-graph cycle, confirmed empirically. The constant-agreement test is a same-repo source-text invariant guard (5 independent `ORDERED_PATH_CHUNK_THRESHOLD`/`SIZE` declarations — memory, postgres, sqlite-backend.ts, sqlite-integrity-assertions.ts, and the new runtime one — per ADR-011's "implementation constant, not protocol constant" framing) and deliberately does not live under `tools/conformance/`, since that machinery is reserved for cross-implementation adapter behavior, not same-language source-constant hygiene.
- **Touched Files (incidental):** `bun.lock` — a 3-line diff removing a stale `@tuvren/kernel-testkit` devDependency lockfile entry for `@tuvren/backend-shared`, whose `package.json` has not declared that dependency since before this ticket began. Confirmed via `git stash` that this drift already existed at this branch's HEAD; a routine `bun install` during implementation regenerated the lockfile and incidentally corrected it. No functional effect.
- **Pre-existing, unrelated gate failure noted (not fixed here):** `bun run verify:kernel`'s final `workspace compatibility evidence check` step (`bun run compatibility:check`) fails with "compatibility matrix typescript-framework checkIds do not match live conformance plan topology" plus related `nonApplicableCheckIds` mismatches for several `*-certification` evidence files. Independently reproduced via `git stash` against this branch's clean HEAD (commit `4381273d`) with KRT-BK008's changes fully removed — confirms this is pre-existing drift orthogonal to TurnTree chunking (most likely from KRT-BK004's conformance plan `planVersion` bump not yet reflected via `bun run compatibility:evidence`), not a regression introduced by this ticket. Flagged here for resolution before the epic's final closing `bun run verify` gate.

**Post-review fix (dual-axis standards + spec review, non-blocking P2, fixed before the next milestone):** the spec reviewer flagged that only single-item appends were tested — `applyStagedResultsToManifest`/`createTurnTree` both support staging multiple results in one call, and no test exercised a single `toStoredTurnTreePathChunkAware` invocation whose delta spans more than one new chunk (i.e., the `for` loop computing >1 new chunk per call was inspected but not executed by any test). Added `runtime-kernel.turn-tree-chunking.test.ts`'s "a single batched incorporate call spanning more than one new chunk resolves correctly" test: grows a prior to a partial-second-chunk state (40 items) via single appends, then incorporates 30 staged results in one call (delta 40→70, spanning the existing partial chunk plus a new one), asserting correct resolution against the unoptimized `tree.create` baseline. Mutation-verified this test is non-redundant: temporarily forced the new-chunk loop to compute a single oversized chunk instead of respecting `RUNTIME_ORDERED_PATH_CHUNK_SIZE`; confirmed this new test — and only this one — failed (the other three chunk-aware tests, which never exceed a 32-item delta, stayed green), then reverted and confirmed byte-identical restoration and all green. Re-verified: `kernel-runtime:test` 56/56 pass, `kernel-runtime:typecheck`/`:lint` clean. The standards review's remaining general-feedback notes (a `Promise.all`-over-tx concurrency pattern observation, and acknowledged/justified duplication between the caller's chunk loop and the backends' normalize loops) were confirmed non-issues and require no code change.
...
##### KRT-BK010 Deviations & Justifications

- **Touched Files (beyond declared Scope):** `rust/kernel/src/types.rs`, `rust/kernel/src/memory.rs`, `rust/kernel/tests/kernel_baseline.rs`, `rust/kernel-conformance-adapter/src/main.rs`, `rust/kernel-conformance-adapter/Cargo.toml`, `Cargo.lock`, `reports/compatibility/evidence/certification-harness.kernel-rust-certification.json`, `reports/compatibility/evidence/certification-harness.framework-typescript-certification.json`, `reports/compatibility/evidence/certification-harness.framework-batteries-included-certification.json`, `reports/compatibility/evidence/certification-harness.framework-rust-certification.json`.
- **Justification:** The ticket's declared Scope anticipated only adapter/plan/matrix files, on the assumption that the Rust kernel already had the state and logic these checks probe. Reading `rust/kernel/src/types.rs` and `memory.rs` directly (before writing any code) showed `RunRecord` had no lease fields at all and `InMemoryKernel` had no `maintenance.reclaim()` primitive — unlike the matrix/adapter/capability files, there was no existing behavior to "just wire up." Promoting `kernel.run-liveness` and `kernel.reclamation` required adding the lease-aware run-liveness API (`run_liveness_create_leased_run`/`renew_lease`/`list_expired`/`preempt_expired`) and a faithful Rust port of the shared TS reachability-reclamation algorithm (`typescript/kernel/backends/shared/src/lib/backend-invariant-reclamation.ts`) directly onto `InMemoryKernel`, plus `created_at_ms` timestamps on `TurnNode`/`ObjectRecord`/`StoredTurnTree` and an `archived_from_branch_id` marker on `BranchRecord` (verified safe against `identity.rs`'s hand-built hash records before adding — none of the new fields participate in content-addressed hashing, so `kernel.protocol.deterministic_hashing`/`schema_roundtrip` are unaffected). `kernel_baseline.rs` needed one `created_at_ms: 0` added to its one direct `TurnNode` literal to keep compiling. The four evidence files changed because `reports/compatibility/compatibility-matrix.json` is a generated, not hand-edited, artifact (per its own declared Scope) — regenerating it via `bun run nx run compatibility-reporting:evidence-refresh` regenerates all adapters' evidence in the same pass, not just rust-kernel's.
- **Scope-text corrections (verified against running code and the actual plan JSON, not assumed):**
  1. **`kernel-restart-af.*` is not restart-dependent and was promoted, not excluded** — this supersedes Gherkin Scenario 2's literal text for these two checks specifically. Reading `spec/conformance/kernel/plans/kernel-run-liveness.json` directly shows `kernel-restart-af.expired-running-requires-preemption` and `kernel-restart-af.paused-run-excluded-from-stale-preemption` invoke the exact same operations (`kernel.run-liveness.stale-preemption`/`.expired-listing`) as the non-restart-af checks, asserting a strict subset of the same result fields, with no per-check `capabilities` override. The compatibility matrix's own `typescript-kernel-memory` reference adapter — pure in-memory, zero durability — already runs and passes both checks today. Once `kernel.run-liveness` is declared, the harness's capability-intersection `selectChecks` logic (`tools/conformance/harness/run.ts`) makes both checks applicable automatically; excluding them would require an identity-based carve-out, which directly violates this ticket's own STOP condition ("never select by identity to make the matrix look better"). Implemented the 3 base run-liveness operations faithfully instead, and the 2 restart-af checks pass as a structural consequence, not a separate effort. `kernel-restart-recovery.close_reopen_checkpoint` (the other check named in the ticket's Out-of-Scope list) is unaffected by this correction and correctly stays excluded — it genuinely requires the `kernel.persistence.durable` capability, which the in-memory Rust kernel correctly does not declare.
  2. **`spec/conformance/interop/rust-kernel/spec/authority-packet.json` (declared in-Scope) was not touched — reading it showed it is the wrong file.** It governs `tuvren.framework.kernel-interop-smoke` (the 9 framework-to-rust-kernel gRPC interop-smoke checks), a different suite entirely from the `kernel.protocol` conformance suite this ticket promotes checks within. The correct authority packet is `spec/kernel/authority-packet.json`; reading it confirmed it is already implementation-neutral (`bindingProjections` already lists `"rust": "rust/kernel"`, and its `reclamation`/`run-liveness` binding-section prose already describes both primitives generically, not per-language), so it required no edit.
  3. **`kernel.reclamation.erasure-probe` was implemented, not deferred**, despite not being named in the ticket's Capability/Contract Mapping prose. Reading `spec/conformance/kernel/plans/kernel-reclamation.json` directly shows none of its 7 checks (including the 2 erasure checks) carry a per-check `capabilities` override — `kernel.reclamation` is all-or-nothing. Declaring it without implementing `erasure-probe` would have made the 2 erasure checks *applicable* and then fail at dispatch (`adapter_operation_not_implemented`), which is worse than not promoting reclamation at all. Reading `typescript/kernel/conformance-adapter/src/host.ts`'s `runErasureProbe` confirmed the crypto is entirely host/adapter-side (the kernel only ever sees opaque ciphertext bytes via `store_put`/`store_get`); implemented the equivalent in `rust/kernel-conformance-adapter/src/main.rs` using a new `aes-gcm = "0.11"` dependency scoped to the adapter crate only (`rust/kernel` itself stays crypto-free).
  4. **`kernel.scope-isolation.*` remains correctly deferred**, so Gherkin Scenario 1's literal text ("scope-isolation checks run against rust-kernel") does not hold as written — only reclamation and run-liveness were promoted this ticket. Corrected framing (a dual-axis spec review caught the first draft of this note overclaiming): this is **not** inherent in-memory inexpressibility — the compatibility matrix's own `typescript-kernel-memory` reference adapter, pure in-memory with zero durability, already passes all three `kernel.scope-isolation.*` checks today, proving the capability is expressible against an in-memory backend in principle. The honest reason is that the Rust `InMemoryKernel`'s `KernelState` has no Scope dimension built at all (verified directly in `rust/kernel/src/memory.rs`), and threading one through roughly 30 methods is a genuine feature addition, not a wiring gap this ticket's effort budget covers — declaring the capability without that work would be exactly the "make the matrix look better" shortcut the ticket's STOP condition forbids. Tracked at `.constitution/tasks/critical-path.md`'s Epic BN entry (named-but-not-yet-ticketed), which now explicitly lists `kernel.scope-isolation.*` promotion as in scope alongside the genuinely durability-dependent checks; Epic BN's KRT-BK010 back-reference there was also corrected to no longer list `kernel-restart-af.*` as deferred, since correction 1 above promoted it.
  5. The ticket Description's "42 of the 68 checks" is a minor pre-existing inaccuracy (the actual total is 70, not 68) — left as-is since correcting historical ticket prose is out of scope; the Gherkin's own success criterion (ratio increases, no regression) is what was verified.
- **Verification performed:** `cargo test -p tuvren-kernel-rust` (13 unit + 40 integration tests, including 5 new reclamation/lease unit tests) and `cargo test -p tuvren-kernel-rust-conformance-adapter` both pass; `cargo build --workspace` and `cargo test --workspace` (6 crates, 64 tests) confirm no sibling crate broke from the new struct fields; `cargo fmt --check` and `cargo clippy --all-targets -- -D warnings` clean on both touched crates. Ran `bun tools/conformance/harness/run.ts --adapter rust/kernel-conformance-adapter/adapter.json` directly (not just the implementer's report) and independently confirmed 54/70 applicable checks pass, 0 fail, 16 non-applicable, and that the passing set is exactly the 12 checks named above (7 reclamation + 3 run-liveness + 2 restart-af) with the prior 42 unchanged. Regenerated `reports/compatibility/compatibility-matrix.json` via `bun run nx run compatibility-reporting:evidence-refresh` and personally diffed the result: only `rust-kernel`'s entry changed semantically (42→54 passed, declaredCapabilities gained `kernel.reclamation`/`kernel.run-liveness`); a `secret-isolation.provider-bridge-request-and-headers` check appeared uniformly across the framework-suite entries as an unrelated, benign catch-up from KRT-BK004's earlier conformance-plan work that had not yet been captured by a matrix refresh — not something this ticket introduced, and it passes cleanly everywhere it now applies. Ran `bun run compatibility:check` (the lane flagged as known-red after KRT-BK008) and confirmed it now passes cleanly — the regeneration resolved that stale-evidence drift rather than entangling with or worsening it.
- **Post-review fix (dual-axis standards + spec review, one mandatory P1 plus non-blocking P2/Investigate findings fixed before the next milestone):**
  - **Standards P1 (mandatory):** `run_erasure_probe`'s post-erasure `unrecoverableAfterErasure` assertion (`rust/kernel-conformance-adapter/src/main.rs`) was a tautology — `key.is_none()` read two lines after `key = None`, so it could never be false and gave zero regression coverage for the check it feeds. Fixed by genuinely attempting decryption of the still-stored ciphertext against a freshly-generated (certainly-wrong) key via the file's existing `decrypt_envelope` helper and asserting the AES-GCM authentication check rejects it, mirroring the TypeScript reference's actual post-erasure decrypt-and-observe-failure pattern instead of inspecting local `Option` state.
  - **Standards Investigate (fixed, cheap and clearly correct):** `run_liveness_renew_lease` checked run status before lease-presence, inconsistent with both the TypeScript reference and this file's own `run_liveness_preempt_expired`. Reordered to check lease-presence first, matching both.
  - **Standards P2 (fixed):** the new lease state machine and reclamation sweep paths had thin unit-test coverage (only the orphan-object sweep and reclamation grace-horizon cases were tested; the lease error branches and the turn/turn-node/archived-branch sweep paths had none). Added 14 unit tests in `rust/kernel/src/memory.rs` covering `run_liveness_renew_lease`'s and `run_liveness_preempt_expired`'s error branches, `run_liveness_list_expired`'s inclusion/exclusion rules, and `maintenance_reclaim`'s archived-branch/turn sweep plus the invariant that a live branch head is never swept regardless of age.
  - **Standards P2 (logged, not fixed):** minor duplicated setup code across the reclamation/leaseless probe scenarios and the three run-liveness probe functions in `main.rs`. Left as-is — this is test/adapter probe code already verified passing; a DRY refactor there trades a style improvement for real risk of disturbing already-correct, review-verified scenario construction, which CLAUDE.md's "no speculative abstraction" guidance weighs against for a non-blocking finding.
  - **Spec P2 (fixed):** correction 4 above (the scope-isolation deferral) originally framed the gap as the Rust kernel being unable to "express scope-confinement," and claimed a follow-up ticket "is filed" when none existed. Reworded to state plainly that the capability is expressible in principle (proven by the TypeScript in-memory reference already passing it) but structurally unbuilt in Rust, and pointed at a corrected, concrete tracking location — `.constitution/tasks/critical-path.md`'s Epic BN entry — rather than an unfiled ticket claim. That entry was also corrected to drop `kernel-restart-af.*` from Epic BN's scope (promoted by correction 1, not deferred) and add `kernel.scope-isolation.*` to it.
  - Re-verified after all fixes: `cargo test -p tuvren-kernel-rust` (27 unit + 40 integration tests), `cargo test --workspace` (6 crates), `cargo fmt --check` and `cargo clippy --all-targets -- -D warnings` on both touched crates, and the conformance harness run directly against the adapter — all clean, still exactly 54/70 passing, 0 failed, 16 non-applicable, no regression.

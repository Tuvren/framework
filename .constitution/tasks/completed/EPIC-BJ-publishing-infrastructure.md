...
- **Post-review hand-off notes for Epic BL (review round 1 — clean pass, no blocking findings; these are downstream tool-choice constraints, not BJ006 defects):**
  - **Tag-reader constraint (KRT-BL002):** the `@experimental` tags live on the re-export *barrel statements*; the canonical declarations in `capability-shapes.ts` are deliberately untagged (per this ticket's Out-of-Scope list). An api-extractor-class tool reads release tags from a symbol's canonical declaration and would see 22 *untagged* exports — tripping the ADR-056 consistency floor or silently classifying them stable. The review independently proved the tags survive verbatim into emitted `.d.ts` under tsc 6.0.2, so a `.d.ts`-comment walker (an ADR-056-sanctioned alternative) works with barrel-level placement. KRT-BL002 must either choose a barrel-compatible tag reader or duplicate the tags at the declaration site when it lands the gate.
  - **`@packageDocumentation` placement (Epic BL doc generation):** the subpath notice block is the *second* `/** */` comment in the barrel (the Apache license header is the first). api-extractor requires `@packageDocumentation` to be the first block comment (`ae-misplaced-package-tag`) and may drop the notice from generated docs. Nothing consumes the tag today (it is the only `@packageDocumentation` in the TS tree); confirm placement compatibility when Epic BL wires doc generation.
...
##### KRT-BJ007 Deviations & Justifications
- **STOP condition 1 (directory names) honored by verification, not guessing:** the full `typescript/**/package.json` set was enumerated before editing. The ticket's `typescript/backend-{memory,postgres,sqlite}/package.json` paths do not exist; the real homes are `typescript/kernel/backends/{memory,postgres,sqlite}/package.json` (workspace names `@tuvren/backend-*` are unchanged). All other ticket paths matched reality.
- **Touched Files (out-of-scope):** `typescript/kernel/grpc-client/package.json` — not in the ticket's In-Scope list because the package did not exist when the ticket was written; ADR-059 §4 (accepted during KRT-BJ002) explicitly adds `@tuvren/kernel-grpc-client` to the publishable leaf set "in scope for KRT-BJ007 publication metadata", so it receives the same four fields as the other 13 publishable manifests.
- **Publishable set as landed (14):** `@tuvren/core`, `@tuvren/sdk`, `@tuvren/backend-{memory,postgres,sqlite}`, `@tuvren/kernel-grpc-client`, `@tuvren/runner-react`, `@tuvren/provider-bridge-ai-sdk`, `@tuvren/stream-{core,sse,agui}`, `@tuvren/mcp-client`, `@tuvren/telemetry-{otel,semconv}`. Each gained `description`, `license: "Apache-2.0"` (matches the root Apache-2.0 LICENSE), `repository` (`git+https://github.com/Tuvren/framework.git` + per-package `directory`), `files: ["dist"]` (every package's exports map is dist-only, verified), and dropped `private: true`. The internal set (runtime, testkits, certification wrappers, repl host, core-types, kernel protocol/runtime, provider-api) is byte-untouched — still `private: true`, no publication fields.
- **STOP condition 2 (private-dep topology) — recorded, no flags flipped (pre-approved execution parameter):** several to-be-published packages have regular `dependencies` on packages this ticket keeps private: `@tuvren/sdk` → `@tuvren/runtime` + `@tuvren/kernel-protocol` + `@tuvren/kernel-runtime`; the three backends and `@tuvren/kernel-grpc-client` → `@tuvren/kernel-protocol`; `@tuvren/runner-react` and `@tuvren/provider-bridge-ai-sdk` → `@tuvren/provider-api` (a package the ticket names in neither list; it stays private). A real registry publish would fail to resolve these. Per the planning decision recorded for this epic, the ticket is followed literally (internal set stays private) and the whole topology class is **recorded as a known issue for Epic BL's freeze-candidate audit (KRT-BL001)**, which must either publish these engine/contract packages as internal-marked registry packages (ADR-057 §5 itself says "`@tuvren/runtime` remains published … but is documented and marked internal", so the authority already anticipates this resolution for runtime) or restructure the dependencies. Epic BJ performs no real publish, so nothing in BJ is blocked by deferring the resolution.
- **Versioning baseline (scheme decision only, per the ticket's Description):** the publishable set versions in lockstep on a single version line anchored to `@tuvren/core`, honoring ADR-037's single-core-version model (leaf packages peer-dep `@tuvren/core` with a tilde range at publish time per ADR-057 §5-consequences). `0.0.0` remains the in-repo placeholder; the first published baseline is computed by KRT-BJ008's release lane and executed by Epic BL — no version bump was performed here.
- **Post-review hand-off notes (review round 1 — clean pass, no blocking findings):**
  - **Telemetry-package enumeration gap (authority alignment, route to freshness pass + Epic BL):** the review observed that ADR-054/ADR-057 §5/ADR-059 §4's shorthand leaf enumerations ("backends, stream adapters, ReAct runner, AI SDK provider bridge, MCP client") never name the telemetry packages, while this ticket's In-Scope list and Description explicitly include `@tuvren/telemetry-otel` and `@tuvren/telemetry-semconv` ("…MCP client, telemetry packages"). Execution follows the ticket (the execution-stage authority): both received publication metadata. `telemetry-otel` is well-anchored (ADR-042 §3 makes it a host-facing leaf sibling of the stream adapters); `telemetry-semconv` is weaker — it is currently consumed only by private packages, so its stable-core membership should be explicitly confirmed or retracted by **KRT-BL001's freeze-candidate audit before the ADR-054/056 freeze snapshot locks the published set** (retracting after the freeze would be a breaking change). The enumeration mismatch itself is a realization note for `tech-spec/changelog.md` at epic close (ADRs stay frozen as decision records).
  - **Per-package LICENSE/README tarball gap (Epic BL, before first real publish):** none of the 14 publishable packages carries a per-package `LICENSE` file and only 2 have a `README.md`; with `files: ["dist"]`, npm auto-includes those files only when physically present, so published tarballs would declare `license: "Apache-2.0"` while shipping no license text (an Apache-2.0 distribution-compliance gap) and most would render empty package pages. Not a BJ blocker (no real publish happens in this epic); Epic BL should add per-package LICENSE copies (or a publish-time root-LICENSE copy step) and minimal READMEs before the first registry publish.
...
##### KRT-BJ008 Deviations & Justifications
- **Tool choice and STOP condition 1 (single-core-version model):** `@changesets/cli` (2.31.0) is the changeset-class tool, scaffolded with its own CLI (`bun add -d @changesets/cli` + `bunx changeset init` — no hand-rolled config). Its default independent-versioning model would indeed allow leaves to drift onto divergent `@tuvren/core` versions, so the STOP condition is resolved **by configuration**: `.changeset/config.json` declares a `fixed: [["@tuvren/*"]]` group, forcing every `@tuvren` workspace package (publishable and private) to version in lockstep — exactly ADR-037's single-instance model — plus `baseBranch: "master"` and `privatePackages: { version: true, tag: false }` (private manifests stay coherent with the lockstep line, keeping the KRT-BJ007 private-dep topology resolvable in Epic BL, but are never tagged for publish).
- **`release-lane.ts` design:** consumes pending changesets, runs `changeset version`, then independently **re-proves the ADR-037 invariant after the bump** (all `@tuvren` workspace manifests share exactly one version; every in-repo `@tuvren/*` dependency/peerDependency range is workspace-protocol or resolves to that single version) and fails loud on violation. It fails loud when no changesets are pending, and never invokes a publish (STOP condition 2: no `changeset publish`, no registry or credential wiring anywhere).
- **`release-check.ts` verified, deliberately not extended:** its current contents are a runtime-version report plus the full `runVerification()` suite — it is the release *gate*, not a version bumper. The lane references it as the operator's next step rather than embedding it, so versioning computation stays fast and the gate stays single-sourced in `verify.ts`.
- **Proof run (reverted, not committed):** with a throwaway core-only patch changeset, `bun run release` computed a lockstep 0.0.1 across all 26 workspace manifests (14 publishable, all identical), passed the invariant validator, generated per-package CHANGELOGs, and exited 0; the no-pending path exits 1 with guidance. The throwaway state was fully reverted — `0.0.0` remains the in-repo placeholder per KRT-BJ007's baseline decision.
- **Touched Files (out-of-scope):** `bun.lock` and the root `package.json` `devDependencies` entry (`@changesets/cli`) — mechanical consequences of installing the chosen tool (the ticket's root-`package.json` scope names only the script entry); `.changeset/README.md` — generated by `changeset init` and kept as the tool's own usage documentation.
- **Prerequisite green-check correction (execution-discovered; evidence owned by KRT-BJ005):** BJ008's verification command (`release-check` = the full verification suite) surfaced a latent red gate none of the earlier, narrower lanes run: `compatibility-report.ts --check` validates checked-in compatibility evidence against the live conformance plan topology, and KRT-BJ005's three new checks had changed that topology without an evidence refresh (BJ005's own `bun run conformance` verify and `bun run check` do not include this gate). Fixed via the sanctioned `bun run compatibility:evidence` refresh lane; the diff is exactly the M5 delta (3 checkIds join the typescript-framework lane's applicable/passed sets and the batteries-included/rust lanes' nonApplicableCheckIds; totals 475→478; no unrelated drift — review-verified). `release-check` then passed end-to-end (exit 0, all phases).
- **Post-review hand-off notes for Epic BL (review round 1 — clean pass, no blocking findings):**
  - **`.changeset/config.json` `access: "restricted"`** is the init default; the stable-core set publishes publicly under Apache-2.0, and scoped packages need `access: "public"` (config-level or per-package `publishConfig`) before a real `npm publish`, or the publish attempts a private-scope release. Epic BL must flip this alongside its registry wiring.
  - **Publish-time range discipline:** the lane's invariant validator accepts `^version` in addition to `~version`/exact/workspace-protocol; ADR-057 §5 specifies a **tilde** range for leaf→core peer-deps at publish time. In-repo everything is `workspace:*` so the branch is never exercised, but Epic BL's publish-time range replacement must emit tilde (and may tighten the validator to match).
  - **Peer-vs-dep classification:** the validator enforces single-version consistency across merged `dependencies`+`peerDependencies` but not that `@tuvren/core` is specifically a *peer* dependency (the ADR-037 single-instance mechanism). All 13 core-consuming leaves currently classify it correctly; the freeze-candidate audit (KRT-BL001) should assert the classification explicitly.

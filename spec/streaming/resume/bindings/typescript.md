...
- The buffer retains the most recent `capacity` sequenced frames, across turns, in arrival order. Turn bookkeeping is pruned as a turn's last frame is evicted, so total memory stays bounded by `capacity`.

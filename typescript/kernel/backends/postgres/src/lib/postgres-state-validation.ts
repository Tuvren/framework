...
// against a `BackendState` projection loaded from PostgreSQL rows rather than
// the memory backend's live maps. `ValidationHelpers` injects the
// lineage-relationship assertions from postgres-db-lineage.js so this module
 ...
 * @throws The injected persistence error with a `postgres_backend_*` code on
 ...
 *   `postgres_backend_turn_tree_path_ordered_count_mismatch`.
 */
...
// by listTurnsByThread in postgres-integrity-assertions.js).

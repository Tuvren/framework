...
// is identical to the PostgreSQL and SQLite backends' copies. See
 ...
 * mutating it in place, with the memory backend's own CBOR lineage decoders
 * injected. `nowMs` only affects whether an expired leaseless running run
 * stops pinning the grace horizon; reachability is clock-independent.
 *

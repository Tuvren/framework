  ...
  test("cancels an aimock-backed multi-iteration turn across all provider families", async () => {
    for (const provider of AIMOCK_PROVIDER_CASES) {
      ...
      try {
        ...
        // cooperative-cancellation forwarding, the in-flight second request's
        // fetch is aborted, so the mock may register one or two requests
        // depending on whether the abort beats the request send.

...
Honesty beats coverage in a manifest. `rust/conformance-adapter/adapter.json` (the Rust framework stub) declares `"capabilities": []` against the same authority packets as the TypeScript framework adapter — so every check is recorded as non-applicable rather than faked. The TypeScript kernel adapter ships three manifests (`adapter.json`, `adapter-sqlite.json`, `adapter-postgres.json`) that run the same host program with different `--backend` flags and capability sets (memory runs the TypeScript source under Bun; sqlite and postgres run the compiled `dist/host.mjs` under Node) — one adapter manifest per implementation/backend combination under test.

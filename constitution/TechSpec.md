...
- v0.29.2 - Epic AX (Tuvren-Server Execution Class) closed: `TuvrenToolDefinition` extended with `outputSchema?`, `idempotent?`, `maxRetries?`; `AgentConfig` extended with `serverExecution?` (rate-limit config) and `sandboxExecutors?`; `TOOL_INPUT_VALIDATION_FAILED`, `TOOL_RESULT_VALIDATION_FAILED`, `TOOL_INVOCATION_RATE_LIMITED` added to `@tuvren/core/errors`; `TuvrenSandboxExecutor` interface in `@tuvren/core/capabilities`; `ServerRateLimiter` (per-turn per-instance fixed-window); `ToolAuditEvent` in `TuvrenStreamEvent`; `tuvren-server-execution-class` conformance check set (18 checks). §1.2 current-capability-orchestration-posture updated.
...
- **Current capability-orchestration posture:** Epics AW and AX are closed. Repository reality now includes: `@tuvren/core/capabilities` subpath with all §3.13 types plus `CapabilityPolicyEngine` and `TuvrenSandboxExecutor` interfaces; Capability Registry, Binding & Endpoint Resolver (back-compat `defineTool` → `tuvren-server / tuvren-in-process`; MCP → `tuvren-server / mcp-server`; `metadata.sandbox.endpointId` → `tuvren-server / tuvren-sandbox`), and Capability Policy Engine wired into tool dispatch; `TuvrenToolDefinition` extended with `outputSchema?`, `idempotent?`, `maxRetries?`; `AgentConfig` extended with `capabilityPolicyEngine?`, `serverExecution?` (rate-limit config), and `sandboxExecutors?`; `TOOL_INPUT_VALIDATION_FAILED`, `TOOL_RESULT_VALIDATION_FAILED`, `TOOL_INVOCATION_RATE_LIMITED` error codes in `@tuvren/core/errors`; `ServerRateLimiter` (fixed-window, per-turn, per-runtime-instance) for tenant-scoped call-budget enforcement; `ToolAuditEvent` (`tool.audit`) in `TuvrenStreamEvent` emitting at five lifecycle points (input_validated, output_validated, retry_attempt, rate_limited, cancelled/reserved) with no secret material; execution-class + owner attribution on events and telemetry; `tuvren-server-execution-class` conformance check set (18 checks, 18/18 pass). 349 runtime tests pass. The remaining Tooling block (Epics AY–BC) deepens the provider-native, provider-mediated, and Tuvren-client execution classes and closes out the model.

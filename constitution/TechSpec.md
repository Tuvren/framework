...
- The hardening line owns testkit extraction, release-check tooling that still includes the private playground host proof, package export smoke tests, and explicit portability-matrix validation for clearly portable core non-native packages across Bun and Node. `constitution/spikes/epic-q-hardening-gap-inventory.md` is the authoritative starting inventory for that work. Epic Q certifies internal implementation-line readiness rather than public package publication. Deno checks remain deferred until package surfaces stabilize enough to avoid testing scaffolding churn.
...
- **Epic Q — Testkit, Portability, and Release Hardening:** Active in current repo reality. `KRT-Q001` is closed and recorded in `constitution/spikes/epic-q-hardening-gap-inventory.md`. The remaining Epic Q scope covers provider-contract-first testkit extraction with the AI SDK bridge as the first proving implementation, stream/control-focused framework testkit extraction that preserves the documented canonical/SSE-only resumed-continuation limit for AG-UI, release-check tooling that still includes the private playground host proof, package export smoke tests, and conservative Bun/Node portability validation through an explicit checked-in matrix. Deno checks remain deferred until package surfaces stabilize enough to avoid scaffolding churn.

...
- **Current capability-orchestration posture:** Epic AW (Capability Orchestration Foundation) is closed. Repository reality now includes: `@tuvren/core/capabilities` subpath with all §3.13 types plus the `CapabilityPolicyEngine` interface; Capability Registry (`capability-registry.ts`), Binding & Endpoint Resolver (`binding-resolver.ts`) with `capability_binding_unavailable` in `@tuvren/core/errors`, and Capability Policy Engine (`capability-policy-engine.ts`) wired into the tool dispatch path (`resolveExecutableToolCall`) so invocation denials surface as `tool.result isError:true`; `AgentConfig.capabilityPolicyEngine?` as the host configuration seam; execution-class + owner attribution additively on `ToolStartEvent` and `ToolResultEvent` plus `tool_call` telemetry spans; semconv extended with `tuvren.runtime.capability.execution_class` and `tuvren.runtime.capability.owner`; authority packet bumped to v1.2.0; and the `runtime-api-capability-orchestration` foundation conformance check set (12 checks, 347/347 framework conformance pass). Back-compat is preserved: `defineTool` resolves to `tuvren-server / tuvren-in-process`, MCP-advertised tools to `tuvren-server / mcp-server`. The remaining Tooling block (Epics AX–BC) deepens each execution class and closes out the model.

...
- v0.13.0 - Closed Epic AD in current repo reality through the docs-to-authority claim inventory, coverage matrix, deferred/local surface decisions, docs cleanup, and freeze gate report; TypeScript remains blocked on Epic AE modular hardening and Epic AF conformance/fresh-evidence closure.
...
- **Freeze-readiness posture:** Passing the currently promoted conformance subsets is necessary but not sufficient for TypeScript freeze-readiness. Epic AD now supplies the checked-in docs-to-authority coverage matrix and explicit classification of remaining implementation-local, implementation-defined, deferred, stale-corrected, or missing-conformance surfaces. Freeze status still requires decomposition of the large TypeScript semantic gravity wells behind stable seams and fresh `bun run verify`, `bun run release-check`, `bun run conformance`, `bun run codegen`, and `bun run interop-smoke` evidence from a clean checkout after Epic AE and Epic AF close.
...
- **Current implementation-line gate posture:** Rust framework product work remains blocked. Epic AD is closed as the docs-to-authority gate, but the current authorized implementation scope still requires Epic AE and Epic AF before TypeScript can be recorded as freeze-ready. Only a later TechSpec revision may activate a new framework implementation line after that closure.
...
- **Later driver, backend, provider, language, and host protocol expansion:** Deferred beyond active Epics AE and AF unless the TechSpec is revised again. This includes Rust framework work, `LanguageModelV2` compatibility, provider-native tools, AI SDK agent loops/UI transports, LangChain bridge work, first-class Tuvren provider packages, ACP, future non-ReAct drivers, future official backends, and future language lines beyond Rust.

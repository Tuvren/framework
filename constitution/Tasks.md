...
- v0.31.6 - Closed Epic BB Exposure & Invocation Policy Model: extended the Capability Policy Engine with five policy dimensions (data-residency, risk-classification, user-presence/active-endpoint, idempotency/retry, credential-boundary) and deterministic composition/precedence at both decision points (exposure-time and invocation-time); added `PolicyDimension` extension interface exported from `@tuvren/runtime`; wired exposure-time filtering into the driver tool registry snapshot path (`applyExposureFilter`); wired `policyCanRetry` into `executeSingleTool` retry path so `non-idempotent` policy suppresses retry regardless of `tool.idempotent`; threaded all five policy-relevant metadata fields (`endpointRegion`, `riskClass`, `requiresUserPresence`, `idempotencyPolicy`, `credentialScope`) through `resolveFromToolDefinition` and the `applyExposureFilter` surface map; added 26-check `capability-policy` conformance plan covering BB001–BB005 at both decision points with non-secret denial reason assertions and determinism checks; registered in `boundaries/shared/contracts/core/spec/authority-packet.json` v1.2.0; 478 runtime tests pass; 425/425 framework conformance checks pass; `bun run verify` exits 0.
...
- **Block 5 — Tooling restructuring (Epics AW–BC): Epics AW, AX, AY, AZ, BA, and BB closed; Epic BC ACTIVE, top priority.** AW delivered the capability-orchestration foundation; AX delivered the full Tuvren-server execution class; AY delivered provider-native and provider-mediated execution classes; AZ delivered the Tuvren-client execution class (runtime-side protocol + attachment seam, client-side MCP, staleness handling, partial observability); BA delivered the invocation lifecycle & observation model; BB delivered the full exposure & invocation policy model. The remainder closes out the cross-class model.
...
| BB | 26 | Delivered Exposure & Invocation Policy Model: five policy dimensions added to `CapabilityPolicyEngine` (data-residency, risk-classification, user-presence/active-endpoint, idempotency/retry, credential-boundary) via a `PolicyDimension` pipeline with deterministic first-deny-wins composition; `PolicyDimension` extension interface exported from `@tuvren/runtime` for host-contributed dimensions; exposure-time filtering wired into the driver tool registry snapshot path (`applyExposureFilter` in `runtime-core-driver.ts`); `policyCanRetry` annotation from the idempotency dimension wired into `executeSingleTool` retry path (`policyCanRetry: false` suppresses retry regardless of `tool.idempotent`); all five policy-relevant metadata fields (`endpointRegion`, `riskClass`, `requiresUserPresence`, `idempotencyPolicy`, `credentialScope`) threaded through `resolveFromToolDefinition` and the exposure-filter surface map; five new shape fields on `ToolSurface`, six on `Endpoint`/`Binding`, six on `CapabilityPolicyContext`, one on `InvocationDecision` (`policyCanRetry`) — all optional/additive; 26-check `capability-policy` conformance plan covering all five BB dimensions at both decision points, composition/precedence, and non-secret denial reasons, registered in the authority packet as an executable verification path; invocation-time policy context wired from real loop state values. 478 runtime tests pass; 425/425 framework conformance checks pass; `bun run verify` exits 0. | `capability-policy` conformance plan (26/26 pass); `boundaries/shared/contracts/core/spec/authority-packet.json` |

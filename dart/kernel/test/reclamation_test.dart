...
void main() {
  ...
  test(
    ...
    () {
      ...
      final sharedMessage = kernel.putObject(
        'application/json',
        'shared-across-live-and-archived'.codeUnits,
      );
      final sharedTree = kernel.createTurnTree('schema_main', {
      ...
      final archivedOnlyMessage = kernel.putObject(
        'application/json',
        'archived-exclusive-payload'.codeUnits,
      );
      final archivedTree = kernel.createTurnTree('schema_main', {
      ...
      expect(
        kernel.hasObject(sharedMessage),
        isTrue,
  ...
  test(
    ...
    () {
      ...
      clock.setMs(10);
      final orphanBeforeLease = kernel.putObject('application/octet-stream', [
        1,
      ]);

      clock.setMs(20);
      final created = kernel.createThread(
        'thread_reclaim_grace',
        'schema_main',
      ...
      clock.setMs(30);
      final orphanAfterLease = kernel.putObject('application/octet-stream', [
        2,
      ]);
      ...
      expect(
        kernel.hasObject(orphanBeforeLease),
        isFalse,
        reason: 'older than the active run that pins the horizon',
      );
      expect(
        kernel.hasObject(orphanAfterLease),
        isTrue,
  ...
  test(
    ...
    () {
      ...
      final orphan = kernel.putObject(
        'application/octet-stream',
        'leaseless-expiry-orphan'.codeUnits,
      );
  ...
  test(
    ...
    () {
      ...
      final orphan = kernel.putObject(
        'application/octet-stream',
        'leaseless-active-orphan'.codeUnits,
      );
  ...
  test(
    'reclaim on a backend that does not implement Reclaimer is unsupported',
    () {
      final kernel = Kernel(
        'scope',
        IncrementingClock(),
        _NonReclaimingBackend(),
      );
